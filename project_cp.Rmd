...
plot <- plot + geom_vline(aes(xintercept=mean_max))+
        theme_bw()
plot
```
...
Using a significance level of $ 1\%$ we can reject the Null hypothesis with a $99\%$ probabilty and state that the last shot in individual races is really more troublesome then in every other race format and that the pressure of loosing a whole minute is the decider in the performance.
